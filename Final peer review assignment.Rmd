...
my_dat <- data.frame(alpha = alpha,beta = beta)
my_dat
...
subset <- filter(dat, gender == 1 & region <= 2 & marstat == 1 & newsint == 1)
str(subset)
...
dat_with_tax_scale <-  mutate(dat, tax_scale = recode(dat$CC18_325a + dat$CC18_325d, "4" = 0, "3" = 1, "2" = 2 ))

dat_with_tax_scale$tax_scale[1:20]
```
...
table(dat_with_tax_scale$tax_scale)
...
group_by_regions <- dat %>% group_by(region)
summarise(group_by_regions,  Trump_Approve_Mean = mean(CC18_308a))
```
...
dat_10 <- filter(dat, investor == 2 & (5 >= faminc_new) & (faminc_new <= 10))
